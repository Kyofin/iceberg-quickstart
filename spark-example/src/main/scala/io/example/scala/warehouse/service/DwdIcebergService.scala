package io.example.scala.warehouse.service

import io.example.scala.warehouse.bean.{BaseWebsite, MemberRegType, VipLevel}
import io.example.scala.warehouse.logDataPath
...
object DwdIcebergService {
  ...
  def readOdsData(sparkSession: SparkSession) = {
    ...
    sparkSession.read.json(s"${logDataPath}/baseadlog.log")
      .withColumn("adid", col("adid").cast("Int"))
      .writeTo("hadoop_prod.db.dwd_base_ad").overwritePartitions()

    sparkSession.read.json(s"${logDataPath}/baswewebsite.log").map(item => {
    ...
    sparkSession.read.json(s"${logDataPath}/member.log").drop("dn")
      .withColumn("uid", col("uid").cast("int"))
      .withColumn("ad_id", col("ad_id").cast("int"))
      .writeTo("hadoop_prod.db.dwd_member").overwritePartitions()

    sparkSession.read.json(s"${logDataPath}/memberRegtype.log").drop("domain").drop("dn")
    ...
    sparkSession.read.json(s"${logDataPath}/pcenterMemViplevel.log").drop("discountval")
      .map(item => {
    ...
    sparkSession.read.json(s"${logDataPath}/pcentermempaymoney.log")
      .withColumn("uid", col("uid").cast("int"))
